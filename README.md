...
- **Agent readability scored, not guessed** — `check:agent-readability` crawls every built app against [Vercel's agent-readability spec](https://vercel.com/kb/guide/agent-readability-spec) (markdown mirrors, content negotiation, AGENTS.md, structured data with freshness signals) — all three apps ship at 90+ ("Excellent") out of the box. → [`docs/stack/agent-readability.md`](./docs/stack/agent-readability.md)
- **CHANGELOG entries are enforced, not optional** — `check:changelog` fails a PR that touches `apps/`, `libs/`, `services/`, `scripts/`, `ops/`, `.wt0*`, `Tiltfile`, or `tilt_*.sh` without adding its own line under `CHANGELOG.md`'s `## Unreleased` in the same diff (docs-only and dependabot PRs are exempt, and say so). → [`docs/stack/changelog.md`](./docs/stack/changelog.md)
- **WebMCP, wired but honest about its status** — `@stack/webmcp` lets a page register agent-callable tools via `document.modelContext` (typed, zod-validated, with a static `/.well-known/webmcp-tools.json` manifest); it's an unshipped browser API behind an origin trial today, not something to build a product flow on yet. → [`docs/stack/webmcp.md`](./docs/stack/webmcp.md)
